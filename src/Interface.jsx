...
import { Camera, AlertCircle, ShoppingCart } from 'lucide-react'
...
export const Interface = ({
  ...
  const { color, strawType, endType, length, diameter, qtyPerSide, innerBoxes, masterCarton, wrapped } = config
  
  const totalQty = (config.qtyPerBox || 0) * (config.boxesPerCarton || 0)
  ...
  const handleAddToQuote = () => {
    if (!isMoqMet) return
    console.log("Add to Quote Payload:", JSON.stringify(config, null, 2))
    alert("Added to Quote! Check Console for JSON.")
  }
  ...
  return (
    <motion.div 
      ...
      className="w-[450px] bg-white h-full shadow-2xl absolute right-0 top-0 overflow-y-auto z-10 font-sans text-gray-800"
    >
      <div className="p-8 space-y-8">
        ...
        <section>
          ...
          <div className="flex gap-4">
             <div className="relative w-48 h-32 rounded-lg overflow-hidden shadow-inner ring-1 ring-black/5 bg-gradient-to-r from-red-500 via-green-500 to-blue-500">
                <input 
                  type="color" 
                  value={color}
           ...
           <div className="flex rounded shadow-sm">
              {['Unwrapped', 'Paper Wrapped', 'Film Wrapped'].map((w, i) => (
                 <button
                   ...
                   className={`flex-1 py-2 text-xs font-medium border-y border-r first:border-l first:rounded-l last:rounded-r border-gray-200 hover:bg-gray-50 transition-colors ${
                      wrapped === w || (!wrapped && i === 0) 
                      ? 'bg-gray-900 text-white font-bold border-gray-900 z-10' 
                      : 'text-gray-500' 
                   }`}
                 >
                    {w}
           ...
           <div className="grid grid-cols-[1fr_auto_1fr_auto_1fr] gap-2 items-end text-center">
              <div className="text-left">
                 ...
                 <input 
                   ...
                   onChange={(e) => handleChange('qtyPerBox', Number(e.target.value))}
                   className="w-full p-2 border border-gray-200 rounded text-center text-gray-700" 
                 />
              </div>
              ...
              <div className="text-left">
                 ...
                 <input 
                   ...
                   onChange={(e) => handleChange('boxesPerCarton', Number(e.target.value))}
                   className="w-full p-2 border border-gray-200 rounded text-center text-gray-700" 
                 />
              </div>
              <div className="pb-3 text-gray-400 font-bold">=</div>
               <div className="text-left relative">
                 ...
                 {!isMoqMet && (
                    <div className="absolute top-full left-0 w-[200px] mt-2 text-[10px] leading-tight text-red-500 font-bold">
